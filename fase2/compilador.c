...
int adiciona_variavel(char *string) {
    ...
    if (topo < 100) {
        variaveis[topo] = strdup(string);  // Aloca e copia a string
        if (variaveis[topo] == NULL) {     // Verifica se a alocação foi bem-sucedida
...
int consome(TAtomo atomo){
    ...
    if(lookahead.atomo == atomo) { // se o tipo de atomo passado na função for igual ao tipo do atomo reconhecido pelo lexer indica que esta de acordo com a gramatica
        if (lookahead.atomo == IDENTIFICADOR){
            temp = adiciona_variavel(lookahead.atributo_ID); // TESTE
            printf("%d:identificador | %s - pos: %d\n",lookahead.linha, lookahead.atributo_ID, temp);
...
void lista_variavel_i(){
    if(consome(IDENTIFICADOR) != -1){
        printf("ERRO \n"); // TODO escrever mensagem de erro
        exit(0);
    }
    while (lookahead.atomo == VIRGULA){
        consome(VIRGULA);
        // TODO: verifica se variavel ja existe
        if(consome(IDENTIFICADOR) != -1){
            printf("ERRO \n"); // TODO escrever mensagem de erro
            exit(0);
        }
